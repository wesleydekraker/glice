...
from dpu_utils.utils import DoubleBufferedIterator
...
class GraphDataset(Generic[GraphSampleType]):
    ...
    def get_default_hyperparameters(cls) -> Dict[str, Any]:
        return {"max_nodes_per_batch": 10000}
    ...
    def graph_batch_iterator(self, data_fold: DataFold) -> Iterator[Tuple[Dict[str, Any], Dict[str, Any]]]:
    ...
    def graph_batch_iterator_from_graph_iterator(self, graph_sample_iterator: Iterator[GraphSampleType]
    ...
    def _batch_would_be_too_full(self, raw_batch: Dict[str, Any], graph_sample: GraphSampleType) -> bool:
    ...
    def _add_graph_to_batch(self, raw_batch: Dict[str, Any], graph_sample: GraphSampleType) -> None:
        ...
        raw_batch["node_to_graph_map"].append(
            np.full(shape=[num_nodes_in_graph], fill_value=raw_batch["num_graphs_in_batch"], dtype=np.int32)
        )
    ...
    def get_batch_tf_data_description(self) -> GraphBatchTFDataDescription:
        ...
        return GraphBatchTFDataDescription(batch_features_types=batch_features_types,
    ...
    def get_tensorflow_dataset(self, data_fold: DataFold) -> tf.data.Dataset:
        ...
        if self._use_worker_threads:
            graph_batch_iterator = lambda: DoubleBufferedIterator(self.graph_batch_iterator(data_fold))
        else:
            graph_batch_iterator = lambda: self.graph_batch_iterator(data_fold)

        dataset = tf.data.Dataset.from_generator(
            generator=graph_batch_iterator,
            output_types=(data_description.batch_features_types, data_description.batch_labels_types),
            output_shapes=(data_description.batch_features_shapes, data_description.batch_labels_shapes))
