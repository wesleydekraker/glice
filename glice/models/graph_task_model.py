...
from glice import GNNInput, GNN
from glice.data import GraphDataset
from glice.utils.polynomial_warmup_and_decay_schedule import PolynomialWarmupAndDecaySchedule
...
class GraphTaskModel(tf.keras.Model):
    ...
    def get_default_hyperparameters(cls, mp_style: Optional[str] = None) -> Dict[str, Any]:
        ...
        these_hypers: Dict[str, Any] = {
            "optimizer": "Adam",  # One of "SGD", "RMSProp", "Adam", "Adagrad", "Adadelta", "Adamax", "Nadam"
            "learning_rate": 0.001,
    ...
    def compute_initial_node_features(self, inputs) -> tf.Tensor:
        return inputs["node_features"]

    @abstractmethod
    def compute_task_output(self,
    ...
    def compute_final_node_representations(self, inputs, training: bool):
        ...
        initial_node_features = self.compute_initial_node_features(inputs)
        gnn_input = GNNInput(
    ...
    def _make_optimizer(self,
        ...
        if optimizer_name == "sgd":
            return tf.keras.optimizers.SGD(learning_rate=learning_rate, momentum=self._params["momentum"])
        elif optimizer_name == "rmsprop":
            return tf.keras.optimizers.RMSprop(learning_rate=learning_rate, momentum=self._params["momentum"],
    ...
    def _apply_gradients(self, gradient_variable_pairs: Iterable[Tuple[tf.Tensor, tf.Variable]]) -> None:
        """
    ...
    # ----------------------------- Training Loop
    def _run_step(self, batch_features: Dict[str, tf.Tensor], batch_labels: Dict[str, tf.Tensor],
    ...
    def _fast_run_step(self, batch_features_tuple: Tuple[tf.Tensor], batch_labels_tuple: Tuple[tf.Tensor],
                       training: bool):
        batch_features = self._unpack_features(batch_features_tuple)
        batch_labels = self._unpack_labels(batch_labels_tuple)

        with tf.GradientTape() as tape:
            task_output = self.call(batch_features, training=training)
    ...
    def run_one_epoch(self, dataset: tf.data.Dataset, training: bool = True) -> Tuple[float, float, List[Any]]:
    ...
    def predict(self, dataset: tf.data.Dataset):
        ...
        for batch_features, _ in dataset:
            task_outputs.append(self.call(batch_features, training=False))
