...
class PolynomialWarmupAndDecaySchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
    """A LearningRateSchedule that uses a polynomial warmup and decay schedule."""

    def __init__(self, learning_rate: Union[tf.Tensor, float], warmup_steps: Union[tf.Tensor, int],
    ...
    def __call__(self, step):
        with tf.name_scope(self.name or "PolynomialWarmupAndDecay"):
            ...
            return tf.cond(is_in_warmup_phase, true_fn=_warmup_learning_rate, false_fn=_decay_learning_rate)
