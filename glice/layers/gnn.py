...
from glice.utils.param_helpers import get_activation_function
...
class GNN(tf.keras.layers.Layer):
    ...
    def _internal_call(self, inputs: GNNInput, training: bool = False):
        ...
        for layer_idx, mp_layer in enumerate(self._mp_layers):
            ...
            cur_node_representations = mp_layer.call(
                MessagePassingInput(node_embeddings=cur_node_representations, adjacency_lists=adjacency_lists),
                training=training,
            )
            ...
            if layer_idx and layer_idx % self._global_exchange_every_num_layers == 0:
                cur_node_representations = self._global_exchange_layers[str(layer_idx)].call(
                    GraphGlobalExchangeInput(
