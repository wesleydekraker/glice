...
from glice.layers import WeightedSumGraphRepresentation, NodesToGraphRepresentationInput

from glice.utils.gather_dense_gradient import gather_dense_gradient
...
class GraphGlobalExchange(tf.keras.layers.Layer):
    ...
    def __init__(self, hidden_dim: int, weighting_fun: str = "softmax", num_heads: int = 4, dropout_rate: float = 0.0):
    ...
    def _compute_per_node_graph_representations(
        ...
        cur_graph_representations = self._node_to_graph_representation_layer.call(
            NodesToGraphRepresentationInput(
        ...
        if training:
            per_node_graph_representations = tf.nn.dropout(per_node_graph_representations, rate=self._dropout_rate)
...
class GraphGlobalMeanExchange(GraphGlobalExchange):
    ...
    def call(self, inputs: GraphGlobalExchangeInput, training: bool = False):
        per_node_graph_representations = self._compute_per_node_graph_representations(inputs, training)
...
class GraphGlobalMLPExchange(GraphGlobalExchange):
    ...
    def call(self, inputs: GraphGlobalExchangeInput, training: bool = False):
        per_node_graph_representations = self._compute_per_node_graph_representations(inputs, training)
        cur_node_representations = self._mlp.call(
