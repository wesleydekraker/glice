...
class WeightedSumGraphRepresentation(NodesToGraphRepresentation):
    ...
    def __init__(self, graph_representation_size: int, num_heads: int,
        ...
        assert weighting_fun.lower() in {"none", "average", "softmax", "sigmoid"},\
    ...
    def call(self, inputs: NodesToGraphRepresentationInput, training: bool = False):
        ...
        if self._weighting_fun not in ("none", "average"):
            scores = self._scoring_mlp.call(inputs.node_embeddings, training=training)  # Shape [V, H]
        ...
        node_reprs = self._transformation_mlp_activation_fun(
            self._transformation_mlp.call(inputs.node_embeddings, training=training)
        )  # Shape [V, GD]
...
class WASGraphRepresentation(NodesToGraphRepresentation):
    ...
    def __init__(self, graph_representation_size: int = 128, num_heads: int = 8,
    ...
    def call(self, inputs: NodesToGraphRepresentationInput, training: bool = False):
        avg_graph_repr = self.__weighted_avg_graph_repr_layer.call(inputs, training)
        sum_graph_repr = self.__weighted_sum_graph_repr_layer.call(inputs, training)
