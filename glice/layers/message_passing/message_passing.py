...
from glice.utils.param_helpers import (
...
class MessagePassing(tf.keras.layers.Layer):
    ...
    def __init__(self, params: Dict[str, Any], **kwargs):
        ...
        self._message_activation_before_aggregation = params.get("message_activation_before_aggregation", False)
    ...
    def _message_function(self, edge_source_states: tf.Tensor, edge_target_states: tf.Tensor,
                          num_incoming_to_node_per_message: tf.Tensor, edge_type_idx: int, training: bool) -> tf.Tensor:
    ...
    def call(self, inputs: MessagePassingInput, training: bool = False):
        ...
        messages_per_type = self._calculate_messages_per_type(adjacency_lists, node_embeddings, training)
        ...
        new_node_states = self._compute_new_node_embeddings(node_embeddings, messages_per_type,
                                                            edge_type_to_message_targets, num_nodes,
                                                            training)  # Shape [V, H]

        return new_node_states

    def _compute_new_node_embeddings(self, cur_node_embeddings: tf.Tensor, messages_per_type: List[tf.Tensor],
                                     edge_type_to_message_targets: List[tf.Tensor], num_nodes: tf.Tensor,
                                     training: bool):
        ...
        aggregated_messages = self._aggregation_fn(data=messages, segment_ids=message_targets,
    ...
    def _calculate_messages_per_type(self, adjacency_lists: Tuple[tf.Tensor, ...], node_embeddings: tf.Tensor,
                                     training: bool = False) -> List[tf.Tensor]:
        ...
        for edge_type_idx, adjacency_list_for_edge_type in enumerate(adjacency_lists):
            ...
            messages = self._message_function(edge_source_states, edge_target_states,
                                              num_incoming_to_node_per_message, edge_type_idx, training)
...
def calculate_type_to_num_incoming_edges(node_embeddings, adjacency_lists):
    ...
    for edge_type_adjacency_list in adjacency_lists:
        ...
        num_incoming_edges = tf.scatter_nd(indices=indices, updates=tf.ones_like(targets, dtype=tf.float32),
