...
class GGNN(GNN_Edge_MLP):
    ...
    * H: output node representation dimension (set as hidden_dim)
    ...
    def _compute_new_node_embeddings(self, cur_node_embeddings: tf.Tensor, messages_per_type: List[tf.Tensor],
        ...
        aggregated_messages = self._aggregation_fn(data=messages, segment_ids=message_targets, num_segments=num_nodes)

        new_node_embeddings, _ = self._recurrent_unit(inputs=aggregated_messages, states=[cur_node_embeddings],
                                                      training=training)
