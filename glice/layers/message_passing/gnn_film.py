...
class GNN_FiLM(GNN_Edge_MLP):
    ...
    * H: output node representation dimension (set as hidden_dim)
    ...
    >>> output = layer.call(MessagePassingInput(node_embeddings, adjacency_lists))
    >>> print(output)
    ...
    def build(self, input_shapes: MessagePassingInput):
        ...
        for i in range(num_edge_types):
            with tf.name_scope(f"edge_type_{i}-FiLM"):
                film_mlp = MLP(out_size=2 * self._hidden_dim, hidden_layers=self._film_parameter_MLP_hidden_layers)
    ...
    def _message_function(self, edge_source_states: tf.Tensor, edge_target_states: tf.Tensor,
        ...
        messages = super()._message_function(edge_source_states, edge_target_states,num_incoming_to_node_per_message,
                                             edge_type_idx, training)

        film_weights = self._edge_type_film_layer_computations[edge_type_idx](edge_target_states, training)
